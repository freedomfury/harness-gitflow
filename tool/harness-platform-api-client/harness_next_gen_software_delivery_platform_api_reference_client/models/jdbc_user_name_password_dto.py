from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="JDBCUserNamePasswordDTO")


@_attrs_define
class JDBCUserNamePasswordDTO:
    """This entity contains the details of the JDBC Username and Password

    Attributes:
        password_ref (str):
        username (str | Unset):
        username_ref (str | Unset):
    """

    password_ref: str
    username: str | Unset = UNSET
    username_ref: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        password_ref = self.password_ref

        username = self.username

        username_ref = self.username_ref

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "passwordRef": password_ref,
            }
        )
        if username is not UNSET:
            field_dict["username"] = username
        if username_ref is not UNSET:
            field_dict["usernameRef"] = username_ref

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        password_ref = d.pop("passwordRef")

        username = d.pop("username", UNSET)

        username_ref = d.pop("usernameRef", UNSET)

        jdbc_user_name_password_dto = cls(
            password_ref=password_ref,
            username=username,
            username_ref=username_ref,
        )

        jdbc_user_name_password_dto.additional_properties = d
        return jdbc_user_name_password_dto

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
