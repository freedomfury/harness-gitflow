from http import HTTPStatus
from typing import Any

import httpx

from ...client import AuthenticatedClient, Client
from ...models.error import Error
from ...models.failure import Failure
from ...models.list_api_keys_1_api_key_type import ListApiKeys1ApiKeyType
from ...models.response_dto_list_api_key import ResponseDTOListApiKey
from ...types import UNSET, Response, Unset


def _get_kwargs(
    *,
    account_identifier: str,
    org_identifier: str | Unset = UNSET,
    project_identifier: str | Unset = UNSET,
    api_key_type: ListApiKeys1ApiKeyType,
    parent_identifier: str,
    identifiers: list[str] | Unset = UNSET,
) -> dict[str, Any]:

    params: dict[str, Any] = {}

    params["accountIdentifier"] = account_identifier

    params["orgIdentifier"] = org_identifier

    params["projectIdentifier"] = project_identifier

    json_api_key_type: str = api_key_type
    params["apiKeyType"] = json_api_key_type

    params["parentIdentifier"] = parent_identifier

    json_identifiers: list[str] | Unset = UNSET
    if not isinstance(identifiers, Unset):
        json_identifiers = identifiers

    params["identifiers"] = json_identifiers

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/apikey",
        "params": params,
    }

    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Error | Failure | ResponseDTOListApiKey:
    if response.status_code == 400:
        response_400 = Failure.from_dict(response.json())

        return response_400

    if response.status_code == 500:
        response_500 = Error.from_dict(response.json())

        return response_500

    response_default = ResponseDTOListApiKey.from_dict(response.json())

    return response_default


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Error | Failure | ResponseDTOListApiKey]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient | Client,
    account_identifier: str,
    org_identifier: str | Unset = UNSET,
    project_identifier: str | Unset = UNSET,
    api_key_type: ListApiKeys1ApiKeyType,
    parent_identifier: str,
    identifiers: list[str] | Unset = UNSET,
) -> Response[Error | Failure | ResponseDTOListApiKey]:
    """Fetches the list of API Keys corresponding to the request's filter criteria.

    Args:
        account_identifier (str):
        org_identifier (str | Unset):
        project_identifier (str | Unset):
        api_key_type (ListApiKeys1ApiKeyType):
        parent_identifier (str):
        identifiers (list[str] | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Error | Failure | ResponseDTOListApiKey]
    """

    kwargs = _get_kwargs(
        account_identifier=account_identifier,
        org_identifier=org_identifier,
        project_identifier=project_identifier,
        api_key_type=api_key_type,
        parent_identifier=parent_identifier,
        identifiers=identifiers,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    *,
    client: AuthenticatedClient | Client,
    account_identifier: str,
    org_identifier: str | Unset = UNSET,
    project_identifier: str | Unset = UNSET,
    api_key_type: ListApiKeys1ApiKeyType,
    parent_identifier: str,
    identifiers: list[str] | Unset = UNSET,
) -> Error | Failure | ResponseDTOListApiKey | None:
    """Fetches the list of API Keys corresponding to the request's filter criteria.

    Args:
        account_identifier (str):
        org_identifier (str | Unset):
        project_identifier (str | Unset):
        api_key_type (ListApiKeys1ApiKeyType):
        parent_identifier (str):
        identifiers (list[str] | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Error | Failure | ResponseDTOListApiKey
    """

    return sync_detailed(
        client=client,
        account_identifier=account_identifier,
        org_identifier=org_identifier,
        project_identifier=project_identifier,
        api_key_type=api_key_type,
        parent_identifier=parent_identifier,
        identifiers=identifiers,
    ).parsed


async def asyncio_detailed(
    *,
    client: AuthenticatedClient | Client,
    account_identifier: str,
    org_identifier: str | Unset = UNSET,
    project_identifier: str | Unset = UNSET,
    api_key_type: ListApiKeys1ApiKeyType,
    parent_identifier: str,
    identifiers: list[str] | Unset = UNSET,
) -> Response[Error | Failure | ResponseDTOListApiKey]:
    """Fetches the list of API Keys corresponding to the request's filter criteria.

    Args:
        account_identifier (str):
        org_identifier (str | Unset):
        project_identifier (str | Unset):
        api_key_type (ListApiKeys1ApiKeyType):
        parent_identifier (str):
        identifiers (list[str] | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Error | Failure | ResponseDTOListApiKey]
    """

    kwargs = _get_kwargs(
        account_identifier=account_identifier,
        org_identifier=org_identifier,
        project_identifier=project_identifier,
        api_key_type=api_key_type,
        parent_identifier=parent_identifier,
        identifiers=identifiers,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    *,
    client: AuthenticatedClient | Client,
    account_identifier: str,
    org_identifier: str | Unset = UNSET,
    project_identifier: str | Unset = UNSET,
    api_key_type: ListApiKeys1ApiKeyType,
    parent_identifier: str,
    identifiers: list[str] | Unset = UNSET,
) -> Error | Failure | ResponseDTOListApiKey | None:
    """Fetches the list of API Keys corresponding to the request's filter criteria.

    Args:
        account_identifier (str):
        org_identifier (str | Unset):
        project_identifier (str | Unset):
        api_key_type (ListApiKeys1ApiKeyType):
        parent_identifier (str):
        identifiers (list[str] | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Error | Failure | ResponseDTOListApiKey
    """

    return (
        await asyncio_detailed(
            client=client,
            account_identifier=account_identifier,
            org_identifier=org_identifier,
            project_identifier=project_identifier,
            api_key_type=api_key_type,
            parent_identifier=parent_identifier,
            identifiers=identifiers,
        )
    ).parsed
