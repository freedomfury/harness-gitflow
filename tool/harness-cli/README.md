...
```bash
cd tool/harness-cli        # or wherever you unpacked the tarball
./scripts/vendor.sh        # generate vendor/ (the three SDKs) from specs/
...
| Project ID     | `HARNESS_PROJECT_ID`      | (defaults to `image_flow`)     |

Either set works — the local-dev names override the pipeline names. Example:

```bash
# Minimal set for local dev (org/project use defaults)
