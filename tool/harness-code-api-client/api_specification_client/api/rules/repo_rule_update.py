...
from ...models.repo_rule_update_body import RepoRuleUpdateBody
...
def _get_kwargs(
    ...
    rule_identifier: str,
    *,
    body: RepoRuleUpdateBody | Unset = UNSET,
    ...
    _kwargs: dict[str, Any] = {
        "method": "patch",
        "url": "/repos/{repo_identifier}/rules/{rule_identifier}".format(
            repo_identifier=quote(str(repo_identifier), safe=""),
            rule_identifier=quote(str(rule_identifier), safe=""),
        ),
        "params": params,
    }
...
) -> OpenapiRule | UsererrorError | None:
    if response.status_code == 200:
        response_200 = OpenapiRule.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[OpenapiRule | UsererrorError]:
    return Response(
