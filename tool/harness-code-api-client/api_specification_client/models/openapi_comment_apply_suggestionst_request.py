from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.pullreq_suggestion_reference import PullreqSuggestionReference


T = TypeVar("T", bound="OpenapiCommentApplySuggestionstRequest")


@_attrs_define
class OpenapiCommentApplySuggestionstRequest:
    """
    Attributes:
        bypass_rules (bool | Unset):
        dry_run_rules (bool | Unset):
        message (str | Unset):
        suggestions (list[PullreqSuggestionReference] | None | Unset):
        title (str | Unset):
    """

    bypass_rules: bool | Unset = UNSET
    dry_run_rules: bool | Unset = UNSET
    message: str | Unset = UNSET
    suggestions: list[PullreqSuggestionReference] | None | Unset = UNSET
    title: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        bypass_rules = self.bypass_rules

        dry_run_rules = self.dry_run_rules

        message = self.message

        suggestions: list[dict[str, Any]] | None | Unset
        if isinstance(self.suggestions, Unset):
            suggestions = UNSET
        elif isinstance(self.suggestions, list):
            suggestions = []
            for suggestions_type_0_item_data in self.suggestions:
                suggestions_type_0_item = suggestions_type_0_item_data.to_dict()
                suggestions.append(suggestions_type_0_item)

        else:
            suggestions = self.suggestions

        title = self.title

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if bypass_rules is not UNSET:
            field_dict["bypass_rules"] = bypass_rules
        if dry_run_rules is not UNSET:
            field_dict["dry_run_rules"] = dry_run_rules
        if message is not UNSET:
            field_dict["message"] = message
        if suggestions is not UNSET:
            field_dict["suggestions"] = suggestions
        if title is not UNSET:
            field_dict["title"] = title

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.pullreq_suggestion_reference import PullreqSuggestionReference

        d = dict(src_dict)
        bypass_rules = d.pop("bypass_rules", UNSET)

        dry_run_rules = d.pop("dry_run_rules", UNSET)

        message = d.pop("message", UNSET)

        def _parse_suggestions(data: object) -> list[PullreqSuggestionReference] | None | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            try:
                if not isinstance(data, list):
                    raise TypeError()
                suggestions_type_0 = []
                _suggestions_type_0 = data
                for suggestions_type_0_item_data in _suggestions_type_0:
                    suggestions_type_0_item = PullreqSuggestionReference.from_dict(suggestions_type_0_item_data)

                    suggestions_type_0.append(suggestions_type_0_item)

                return suggestions_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(list[PullreqSuggestionReference] | None | Unset, data)

        suggestions = _parse_suggestions(d.pop("suggestions", UNSET))

        title = d.pop("title", UNSET)

        openapi_comment_apply_suggestionst_request = cls(
            bypass_rules=bypass_rules,
            dry_run_rules=dry_run_rules,
            message=message,
            suggestions=suggestions,
            title=title,
        )

        openapi_comment_apply_suggestionst_request.additional_properties = d
        return openapi_comment_apply_suggestionst_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
