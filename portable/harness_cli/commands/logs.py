...
def _common_params(ctx):
    """Return the standard query params (omits unset org/project)."""
    params = {"accountIdentifier": ctx.obj["account_id"]}
    if ctx.obj["org_id"]:
        params["orgIdentifier"] = ctx.obj["org_id"]
    if ctx.obj["project_id"]:
        params["projectIdentifier"] = ctx.obj["project_id"]
    return params
